...
if ($auth_user["show_junk_enabled"]) {
	...
	$row = sql("select * from (select * from (select comments_clean as comments, title, slug, publish_time from story order by publish_time desc limit 100) as last_hundred order by comments desc, publish_time desc limit 5) as top_five order by publish_time desc");
}
